...
- The template-validation step uses
  `nlt-otoi/tools/validators/toi-validator.py` without `--schema`.
- Its default schema path is
  `nlt-otoi/schemas/v1.0/personal-toi-v1.json`.
- That schema validates the nested `nlt-otoi/templates/personal-toi/*.json`
  template shape (`metadata`, `user_profile`, `interaction_preferences`, and
  related support sections). It is intentionally forward-compatible and allows
  unknown properties.
- This is not the canonical `.toi` schema from `@neurolift-technologies/toi`;
  use the package-provided canonical schema when validating new `.toi`
  documents.
...
| `❌ Error: Schema file not found at .../nlt-otoi/schemas/v1.0/personal-toi-v1.json` | The nested personal-TOI template schema is missing, ignored, or the validator path regressed | Confirm `nlt-otoi/schemas/v1.0/personal-toi-v1.json` is tracked and that the validator default path still climbs from `tools/validators/` to `nlt-otoi/schemas/v1.0/` |
...
- If template validation fails, run:
  ...
  ```
- If template validation fails with "Schema file not found", confirm the
  expected default schema exists at
  `nlt-otoi/schemas/v1.0/personal-toi-v1.json`. The nested `.gitignore`
  intentionally ignores user files like `personal-toi-*.json` but re-includes
  this schema definition with `!schemas/v1.0/personal-toi-v1.json`; keep that
  exception if privacy-oriented ignore rules are edited.
