...
Format reference: `NeuroLift-Technologies/nlt-otoi` templates directory.
```
...
Format reference: `templates/escalation.md` in `NeuroLift-Technologies/nlt-otoi`.
```

### Step 5: Grant GitHub App Access to `nlt-otoi`

Coding agents (e.g., GitHub Copilot, Codex) must be able to read the governance files in
`nlt-otoi` from within the new repository. If the GitHub App installation is scoped
to **"Selected repositories"**, you must add `nlt-otoi` to its access list.
...
3. Under **Repository access → Selected repositories**, add **`nlt-otoi`**.
4. Click **Save**.
...
> in `nlt-otoi` for full instructions.
...
- [ ] GitHub App has access to `nlt-otoi` (or public mirror fallback is in place)

---

## Validation

Run the org-level validation script to check `nlt-otoi` is healthy:

```bash
git clone https://github.com/NeuroLift-Technologies/nlt-otoi
cd nlt-otoi
